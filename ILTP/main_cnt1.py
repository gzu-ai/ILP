...
def record_singleton(type_str,time,ram,len,match,timeout,lenCnt):
    global record_list
    record_list.extend([type_str, len, time, ram, match, timeout, lenCnt])
...
def gen_E_B_H_check_new_compare():
    ...
    def recordResult(t_str):
        global decoratorList
        record_singleton(t_str, decoratorList[0], decoratorList[1], len(LastSol.Rules), str(match_flag), timeout_flag, rule_Cnt)
        if silent_level < 4:
...
if __name__ == "__main__":
    GRN_name = 'tcrNet'
    num_of_Is = 300 #{40, 80, 120, 160, 200, 240, 280, 320, 360, 400, 440, 480}
    ...
    Is_amount_list = [5000, 6000, 7000, 8000,9000, 10000] #[100, 1000, 10000]   [500, 1500, 2000, 2500] [3000, 3500, 4000, 4500, 5000] [720, 800]
    Bs_ratio_list = [0.1, 0.4, 0.8]
    num_of_Bs = 0 
    times = 1  # run 10 times and get the average. 2 fro margin.
