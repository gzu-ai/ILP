...
def record_singleton(type_str,time,ram,len,match,timeout):
    global record_list
    record_list.extend([type_str, len, time, ram, match, timeout])
...
def gen_E_B_H_check_new_compare():
    ...
    def recordResult(t_str):
        global decoratorList
        record_singleton(t_str, decoratorList[0], decoratorList[1], len(LastSol.Rules), str(match_flag), timeout_flag)
        if silent_level < 4:
    ...
    def computeMatchRecordNew(t_str):
        facts(t_str)
...
if __name__ == "__main__":
    GRN_name = 'tcrNet'
    num_of_Is = 2 #{40, 80, 120, 160, 200, 240, 280, 320, 360, 400, 440, 480}
    ...
    search_type_list = 'optimalLeastSPSep'
    silent_level = 5
    file_name = 'log.log'
    ...
    type_file_dict = {'optimalBound':'MHSC','optimalLeast':'MHSC-cut-uniform','optimalConstraintR':'R-ILTP','optimalConstraintSP':'SP-ILTP','specificTrimed':'ILTP',
                      ...
                      'optimalApplicableSP':'SP-ILTP-app', 'optimalBoundSep':'MHSC-head', 'optimalLeastSep':'MHSC-cut-uniform-head','optimalLeastSPSep':'MHSC-cut-uniform-SP-head',
    ...
    GRN_name_list = ['tcrNet','thelper','arabidopsis',  'budding', 'fission', 'mammalian'] #['tcrNet', 'thelper','arabidopsis',  'budding', 'fission', 'mammalian'] 不确定边界时，倒序更合理
    ...
    Is_amount_list = [1000, 1200] #[100, 1000, 10000]   [500, 1500, 2000, 2500] [3000, 3500, 4000, 4500, 5000]
    Bs_ratio_list = [0.1, 0.4, 0.8]
    num_of_Bs = 0 
    times = 11  # run 10 times and get the average. 2 fro margin.
