...
class ASP:
    ...
    def computeLastOptimal(self,head_atom,magnification,assigned_dir):
        if head_atom == '':
            file_path = assigned_dir + '/MHSC_v0.lp'
        else:
...
def record_singleton(type_str,time,ram,len,match):
    ...
    if float(time) > 1800 :  # timeLimit_value 30 * 60 = 1800
        timeout = True
    else:
        timeout = False
...
def gen_E_B_H_check_compare(): #search_type == compare
    ...
    def getOptimalParallel():
        def getCMDs():
            ...
            parameter_path = '--outf=0 --verbose=1 --parallel-mode=2,compete --time-limit=1800 --models 0 --opt-mode=opt --quiet=1' # timeLimit_value
...
if __name__ == "__main__":
    ...
    search_type = 'optimal_batch' #search_type in {specific, optimal_batch, optimal_sep, optimal_parallel, overall, compare}
    silent_level = 2
    file_name = 'optimal_V0.log'
